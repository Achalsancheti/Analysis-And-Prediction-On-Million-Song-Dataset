package neu.longestDurationSong;
...
public class LongestDurationSongMapReduce {
	public static class LDMapper
	extends Mapper<LongWritable, Text, FloatWritable, Text>{
		...
		public void map(LongWritable key, Text value, Context context
			...
			if(songId.indexOf("SO")!=-1){
				try{
					float k=Float.parseFloat(words[12].trim());
					if(k>0){
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(LongestDurationSongMapReduce.class);
		job.setMapperClass(LDMapper.class);
		

		job.setReducerClass(LDReducer.class);
